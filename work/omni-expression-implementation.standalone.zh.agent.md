...
## 二点五、基于 CLAUDECODE 的行为约束

以下约束用于降低表达式/函数实现任务中的常见代理失误。默认取向是“谨慎优先于速度”，但对特别简单、边界清晰的任务可以保持合理简洁。

### 1. 先思考，再编码

- 在实现前，必须先明确当前假设，不要把推断当成事实。
- 如果需求存在多种合理解释，不要静默选一种，必须先写明分歧点，再根据证据选择，必要时向用户确认。
- 如果存在更简单的实现路径，必须优先考虑，并明确说明为什么它足够解决当前问题。
- 如果有不清楚的前提、契约或运行方式，不要硬做；必须先指出不清楚的点，再补证据或提问。

### 2. 简单方案优先

- 默认选择能够满足需求的最小实现，不做推测性扩展。
- 不要为一次性使用的逻辑引入额外抽象。
- 不要加入用户没有要求的“可配置性”“通用性”或预留开关。
- 不要为不可能发生的场景添加复杂错误处理。
- 如果实现明显比问题本身复杂，必须主动回退到更直接的方案。

### 3. 手术式修改

- 只修改完成当前需求所必须触及的代码或文档。
- 不要顺手改动相邻代码、注释、格式或做无关重构。
- 保持与原文件现有风格一致，即使你个人会写成别的样子。
- 如果发现无关死代码或历史问题，可以在结论中指出，但不要擅自删除。
- 只清理由本次修改直接产生的无用 import、变量、函数或文档内容；不要顺手清理既有遗留项。
- 自检标准：每一处改动都必须能直接追溯到用户请求或本次验证所需的最小修复。

### 4. 目标驱动执行

- 把任务改写成可验证的目标，而不是模糊地“把它做对”。
- 多步骤任务必须明确写出“步骤 -> 验证方式”的短计划，然后按验证结果迭代。
- 示例：

```text
1. 修改 planner 翻译逻辑 -> 验证：Calc description 不再出现 INVALID
2. 修改 validator 或 native 实现 -> 验证：focused UT 通过
3. 进行 Flink SQL 端到端验证 -> 验证：原生 Flink 与 native 结果一致
```

- 遇到“加校验”“修 bug”“重构某段逻辑”这类请求时，必须把它们落成可执行检查，例如：
   - “加校验” -> 先补非法输入测试，再让测试通过
   - “修 bug” -> 先构造复现，再修复并验证
   - “整理实现” -> 先确认前后行为一致，再比较验证结果
- 如果成功标准仍然是“看起来可以”或“应该没问题”，说明任务定义还不够强，必须继续收紧到可验证标准。
